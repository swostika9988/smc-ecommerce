            ...
            <tr>
                ...
                <td class="border p-2 grid gap-2">
                    <a href="{{ route('order.status', [$order->id, 'Pending']) }}"
                        class="bg-blue-500 text-white px-2 py-1 rounded-lg text-center">Pending</a>
                    <a href="{{ route('order.status', [$order->id, 'Processing']) }}"
                        class="bg-yellow-500 text-white px-2 py-1 rounded-lg text-center">Processing</a>
                    <a href="{{ route('order.status', [$order->id, 'Delivered']) }}"
                        class="bg-red-500 text-white px-2 py-1 rounded-lg text-center">Delivered</a>
                </td>
            </tr>
        @endforeach
